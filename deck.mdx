import { Head, Image } from 'mdx-deck'
import { Notes } from 'mdx-deck'
import Start from './src/Components/start'
import Source from './src/Components/source'
import Video from './src/Components/video'
export theme from './src/theme'
import Invert from './src/layouts/Invert'
import ImgLayout from './src/layouts/image'


<Head>
  <title>Fonts 2019</title>
  <link href="https://fonts.googleapis.com/css?family=Work+Sans:400,700,900" rel="stylesheet" />
  <link href="./styles.css" rel="stylesheet" />
</Head>


export default Start


<Video
  loop={true}
  src="https://cdn.jsdelivr.net/gh/signalwerk/talk.fonts2019@master/img/intro/intro.mp4"
/>

---

## *2019 · State of the Font*
# Irgendwas ist ja immer…



Schule für Gestaltung · Zürich <!-- .element: className="footer" -->


---

## **Stefan Huber** · Entwickler & Dozent
---


export default ImgLayout

<div className="center">



\ * The production of new types <br />is only a ‘necessity’ within capitalism. *
### **Tschichold** · 1931

</div>



<Source>

Active Literature: Jan Tschichold and New Typography, Hyphen Press <br /> Letter from Tschichold to Albers (8. 12. 1931)

</Source>


---



export default Invert


![Polo](./img/SVG/impression-polo.svg)


<Source>


Font: Polo Book · Georg Salden · 1972 <br /> Info: FF Meta · Spiekermann · 1991 | Syntax · Meier · 1968


</Source>





---


export default Invert


![Polo](./img/SVG/impression-lexicon.svg)


<Source>

Font: Lexicon · Bram de Does († 2015) · 1992

</Source>





---


export default Invert


![Polo](./img/SVG/impression-thesis.svg)


<Source>

Font: Thesis · TheMix Plain · Luc(as) de Groot · 1994


</Source>





---




# Fonts · **2019** · *Web*

---
# ✨ Wunschliste ✨


---
## Kerning
---

export default ImgLayout

## ~1445 · Handsatz

![Kerning Bleisatz](https://cdn.jsdelivr.net/gh/signalwerk/talk.fonts2019@master/img/kerning/IMG_9713_sh.png)


<Source>

Bildquelle: Stefan Huber · 2018

</Source>

---
export default ImgLayout

## ~1445 · Handsatz


![Kerning Bleisatz](https://cdn.jsdelivr.net/gh/signalwerk/talk.fonts2019@master/img/kerning/IMG_9862_sh.png)


<Source>

Bildquelle: Stefan Huber · 2018

</Source>


---



export default Invert

## Digitalisierung

![Polo](./img/SVG/Kerning-Class-01.svg)


<Source>

Font: Paratype · PT Sans · Bold <br />

</Source>
---



export default Invert

## 1984 · Adobe PostScript

![Polo](./img/SVG/Kerning-Class-02.svg)


<Source>

Font: Paratype · PT Sans · Bold <br />



</Source>
---



export default Invert

## Globalisierung

![Polo](./img/SVG/Kerning-Class-03.svg)


<Source>

Font: Paratype · PT Sans · Bold


</Source>
---



export default Invert

## OpenType · 1996

![Polo](./img/kerning/Kerning-class-preview.png)


<Source>

Font: Paratype · PT Sans · Bold


</Source>
---




export default Invert

## OpenType · 1996

![Polo](./img/SVG/Kerning-Class-04.svg)


<Source>

Font: Paratype · PT Sans · Bold


</Source>

---



export default Invert


![Polo](./img/SVG/Kerning-01.svg)


<Source>

Font: Adobe · Garamond Premier Pro · Regular


</Source>



---

export default Invert


![Polo](./img/SVG/Kerning-02.svg)


<Source>

Font: Adobe · Garamond Premier Pro · Regular


</Source>

---



export default Invert


![Polo](./img/SVG/Kerning-03.svg)


<Source>

Font: Adobe · Garamond Premier Pro · Regular


</Source>

---




export default Invert


![Polo](./img/SVG/Kerning-04.svg)


<Source>

Font: Adobe · Garamond Premier Pro · Regular


</Source>

---




export default Invert


![Polo](./img/SVG/Kerning-05.svg)


<Source>

Font: Adobe · Garamond Premier Pro · Regular


</Source>

---




export default Invert


![Polo](./img/SVG/Kerning-06.svg)


<Source>

Font: Adobe · Garamond Premier Pro · Regular


</Source>
---

## Kerning · **bitte mehr!**



---
## Zahlensätze
---

<iframe
  className="browser noBorder"
  src="https://signalwerk.github.io/fontkit-demo/?url=AdobeVFPrototype.otf&hideInput=true&sampleText=Typ 6917.10&fontSize=300">
</iframe>


<div className="small center">Font: Adobe VF Prototype</div>

---

## Zahlensätze · **bitte vier!**

---

## Optische Grössen


---
## Variable Fonts · 2016

---

<iframe
  className="browser noBorder"
  src="https://signalwerk.github.io/fontkit-demo/?url=AmstelvarAlpha-VF.ttf&hideInput=true&sampleText=HALLO&fontSize=350">
</iframe>


<div className="small center">Font: Amstelvar Alpha</div>
---

## Optische Grössen · **bitte ja!**


---

## Mikrotypografie


---

## Leerzeichen

---




export default Invert


![Cast](./img/SVG/Spaces-Cast-01.svg)


<Source>

Font:  Cast Medium · Dominique Kerber · 2011


</Source>

---




export default Invert


![Cast](./img/SVG/Spaces-Cast-02.svg)


<Source>

Font:  Cast Medium · Dominique Kerber · 2011


</Source>

---




export default Invert


![Cast](./img/SVG/Spaces-Cast-03.svg)


<Source>

Font:  Cast Medium · Dominique Kerber · 2011


</Source>

---




export default Invert


![Cast](./img/SVG/Spaces-Cast-04.svg)


<Source>

Font:  Cast Medium · Dominique Kerber · 2011


</Source>

---




export default Invert


![Cast](./img/SVG/Spaces-Cast-05.svg)


<Source>

Font:  Cast Medium · Dominique Kerber · 2018


</Source>
---




export default Invert


![Cast](./img/SVG/Spaces-Cast-06.svg)


<Source>

Font:  Cast Medium · Dominique Kerber · 2018


</Source>
---




export default Invert


![Cast](./img/SVG/Spaces-Cast-07.svg)


<Source>

Font:  Cast Medium · Dominique Kerber · 2011/2018


</Source>
---




export default Invert


![Cast](./img/SVG/Spaces-Cast-08.svg)


<Source>

Font:  Cast Medium · Dominique Kerber · 2011/2018


</Source>

---




export default Invert


![Spaces](./img/SVG/Spaces.svg)


<Source>

Font:  Open Sans Regular · Steve Matteson · 2011


</Source>




---


[→ ups...](https://www.apple.com/chde/ipad-pro/specs/)



---

## Mikrotypografie · **2019!**



---

## Zeichenersetzung



---

<iframe
  className="browser noBorder"
  src="https://signalwerk.github.io/fontkit-demo/?url=AdobeVFPrototype.otf&hideInput=true&sampleText=fi fl ffi&fontSize=300">
</iframe>


<div className="small center">Font: Adobe VF Prototype</div>


---

export default Invert



<Video
  loop={true}
  src="https://cdn.jsdelivr.net/gh/signalwerk/talk.fonts2019@master/img/replace/SansBullshitSans/text-entry.mp4"
/>




<Source>

Font: [Sans Bullshit Sans](http://www.sansbullshitsans.com/) · Roel Nieskens · 2015


</Source>




---

## Emojis

---



<iframe
  className="browser"
  src="https://www.webfx.com/tools/emoji-cheat-sheet/">
</iframe>


---

## Emojis mit OpenType-Ersetzung



<iframe
  className="browser noBorder"
  src="https://cdn.jsdelivr.net/gh/signalwerk/talk.fonts2019@master/img/replace/Logical/SVG/cat-animation.svg">
</iframe>



<div className="small center">Font: Logical · Edgar Walthert · 2018</div>


---


:emoji:
emoji in markdown
https://emoji-cheat-sheet.com/
:emoji:  >>> https://logical.family/
https://github.com/WebpageFX/emoji-cheat-sheet.com



chartwell

---



# Zeichen >>> noto

## Sprachabdeckung

* Noto
* Globalfonts >>> Noto
* Unicode >> georgisch



OpenType is a game changer.
So epic and still beta.








---

EXPAND

* Grafik – Verhältnis zwischen – Engeneering zu Technik
* Crazy variable fonts für perfektes typesetting








---

OPTIONAL
* Studi-Arbeit einbauen
* Ausgabe – Darstellung von S/W-Font screen & print
* Ausgabe – Darstellung von farbigem font screen & print 4f/2f – https://www.youtube.com/watch?v=BGK_apl8SUg
* Arabisch Problem
* lokalisierte formen sind in den fonts drin!












---
Lizenz
---

Beste fonst gratis

---
Verschleiss von schriften >>> Lizenzen, die ablaufen

---

Variabler Font
https://logrinto.github.io/IAD2017.variable-fonts/

wie die achsen zuwachsen x/y unterschiedlich

https://developer.mozilla.org/en-US/docs/Web/CSS/CSS_Fonts/OpenType_fonts_guide#Font_variant_shorthand_(font-variant)
spannendes typo-system:  https://rwt.io/





---

super variable fonts


---


Tracking verschiedene formate


CAST FONT


## Abstract
Im Talk werden in schnellen Schritten gezeigt, wo wir heute mit **Fonts im Web** stehen und wie Technologie und Design stark in einandergreifen müssen um gute Typografie zu erreichen. Dafür werden einige Aspekte gezeigt, die bereits erreicht wurden und was noch entwickelt werden muss.

### Ist
* Qualität – Technische und die gestaltereische Qualitäten der Schrift und der Schriftausgabe
* Zeichen – Welche Zeichen setzen wir und wo ist Schrift die Technik für andere Ideen
* Lizenz – Was passt und wem passt es nicht
* Standards – Wer will denn heute noch Standards?

### Ausblick
* Varriable Fonts – Parametrisches Design für den Endanwender
* OpenType 2.0?
  –– DEFAULT Zeichensatz
  –– vorward and backward compatibility
  –– color font



https://v-fonts.com/fonts/venn-vf
https://speakerdeck.com/bramstein/line-breaking-justification-and-variable-fonts
https://mailchi.mp/68f07c3a5247/web-typography-news-1659549


---


`Your rights to use this font software expire on December 31, 2019.`

>> Your rights to use this font software expire on December 31, 2019.
https://www.monotype.com/fonts/variable-fonts


http://www.smeltery.net/fr/fontes/megalopolis-extra
---

---
# Fragen?




---


# Merci




---

## exit 0;
